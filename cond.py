...
def execute(line):
    dest, op, val, _, src, test, test_val = line.split(" ")
    val = int(val)
    test_val = int(test_val)
    if dotest(src, test, test_val):
        if op == "inc":
            #print(op, dest, val)
            regs[dest] += val
        elif op == "dec":
...
def dotest(src, test, test_val):
    if test == ">" and regs[src] > test_val:
        return True
    elif test == "<" and regs[src] < test_val:
        return True
    elif test == "<=" and regs[src] <= test_val:
        return True
    elif test == ">=" and regs[src] >= test_val:
